...
ob <- load("cache/tw_02.05.2017_11pm.RData")
...
tw$text <- sapply(tw$text, trim.whitespace)

### transfer to Pacific time
tw$datetime <- as.POSIXlt(tw$created_at, format="%Y-%m-%d %H:%M:%S")
tw$datetime <- as.POSIXct(tw$datetime, tz="America/Los_Angeles", usetz=TRUE)
tw$datetime <- as.POSIXlt(tw$datetime, format="%b %d %H:%M")
...
out <- count(d, c("date", "hour"))
plot(1:nrow(out), y=out$freq, type = "l", col="#458b74", lwd=3, 
     xlab="Hours", ylab="Unique Tweets", main="#actuallivingscientist")
abline(v=c(11, 35, 59), col="grey", lwd=3)
...
ggplot(d, aes(as.factor(hour)))
+ geom_bar()
+ facet_grid( . ~ date)
+ theme_bw() + ggtitle("Number of Tweets by Hour")
